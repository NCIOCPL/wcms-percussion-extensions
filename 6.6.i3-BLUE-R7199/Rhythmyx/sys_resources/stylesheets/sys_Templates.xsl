...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:psxctl="URN:percussion.com/control" xmlns:xmns="http://www.w3.org/1999/xhtml" xmlns:psxi18n="urn:www.percussion.com/i18n" exclude-result-prefixes="psxi18n psxctl xmns">
    <xsl:import href="file:sys_resources/stylesheets/sys_I18nUtils.xsl"/>
    <xsl:output method="xml" indent="yes"/>
    <xsl:variable name="itemLocale" select="/ContentEditor/@itemLocale"/>
    <xsl:variable name="pssessionid" select="/ContentEditor/UserStatus/@sessionId"/>
    ...
    <xsl:template match="/">
        <html>
            <head>
                ...
                <script type="text/javascript" src="../sys_resources/js/textedit.js">;</script>
            </head>
            <body>
        ...
        <input type="radio" name="{$paramName}" value="{Value}">
            <xsl:choose>
                <xsl:when test="$count = 1">
                    <xsl:attribute name="id"><xsl:value-of select="concat('ce_',$paramName)"/></xsl:attribute>
                </xsl:when>
                 <xsl:otherwise>
                    <xsl:attribute name="id"><xsl:value-of select="concat('ce_',$paramName,'_',$count)"/></xsl:attribute>
                 </xsl:otherwise>
            </xsl:choose>
...
>
 -->
    ...
    <xsl:template match="Control[@name='sys_EditBox']" mode="psxcontrol">
        <input type="text" name="{@paramName}" id="ce_{@paramName}" value="{Value}">
            <xsl:if test="@accessKey!=''">
 ...
 -->
    ...
    <xsl:template match="Control[@name='sys_File']" mode="psxcontrol">
        ...
        <input type="file" name="{@paramName}" id="ce_{@paramName}">
            <xsl:if test="@accessKey!=''">
                <xsl:attribute name="accesskey"><xsl:call-template name="getaccesskey"><xsl:with-param name="label" select="preceding-sibling::DisplayLabel"/><xsl:with-param name="sourceType" select="preceding-sibling::DisplayLabel/@sourceType"/><xsl:with-param name="paramName" select="@paramName"/><xsl:with-param name="accessKey" select="@accessKey"/></xsl:call-template></xsl:attribute>
            </xsl:if>
            <xsl:call-template name="parametersToAttributes">
         ...
         &nbsp;<xsl:value-of select="DisplayLabel"/>
                ...
                <xsl:otherwise>
                    ...
                    <input name="{$Control/@paramName}" type="checkbox" value="{Value}">
                        ...
                        <xsl:choose>
                            <xsl:when test="$count = 1">
                                <xsl:attribute name="id"><xsl:value-of select="concat('ce_',$Control/@paramName)"/></xsl:attribute>
                            </xsl:when>
                             <xsl:otherwise>
                                <xsl:attribute name="id"><xsl:value-of select="concat('ce_',$Control/@paramName,'_',$count)"/></xsl:attribute>
                             </xsl:otherwise>
                        </xsl:choose>
                        <xsl:value-of select="DisplayLabel"/>
                    </input>
...
<!ATTLIST textarea
  ...
  -->
    ...
    <xsl:template match="Control[@name='sys_TextArea']" mode="psxcontrol">
        <textarea name="{@paramName}" id="ce_{@paramName}" wrap="soft">
         <xsl:if test="@accessKey!=''">
            <xsl:attribute name="accesskey"><xsl:call-template name="getaccesskey"><xsl:with-param name="label" select="preceding-sibling::DisplayLabel"/><xsl:with-param name="sourceType" select="preceding-sibling::DisplayLabel/@sourceType"/><xsl:with-param name="paramName" select="@paramName"/><xsl:with-param name="accessKey" select="@accessKey"/></xsl:call-template></xsl:attribute>
         </xsl:if>
            <xsl:call-template name="parametersToAttributes">
  ...
  -->
    <psxctl:ControlMeta name="sys_CalendarSimple" dimension="single" choiceset="none">
        ...
        <psxctl:AssociatedFileList>
            ...
            <psxctl:FileDescriptor name="datePatternValidation.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/datePatternValidation.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
        </psxctl:AssociatedFileList>
    </psxctl:ControlMeta>
    <xsl:template match="Control[@name='sys_CalendarSimple']" mode="psxcontrol">
        <input type="text" name="{@paramName}" id="ce_{@paramName}" value="{Value}" onblur="validateDatePattern(this);">
            <xsl:if test="@accessKey!=''">
                <xsl:attribute name="accesskey"><xsl:value-of select="@accessKey"/></xsl:attribute>
            </xsl:if>
            <xsl:call-template name="parametersToAttributes">
            ...
            <xsl:attribute name="onChange"><xsl:text>fixTimePart(this);</xsl:text></xsl:attribute>
        </input>&#160;
   ...
   <!-- 
    ...
    <psxctl:ControlMeta name="sys_EditLive" dimension="single" choiceset="none">
        ...
        <psxctl:ParamList>
           ...
           <psxctl:Param name="dlg_width" datatype="Number" paramtype="generic">
              <psxctl:Description>This parameter specifies the width of the dialog box that is opened during field editing in Active Assembly.</psxctl:Description>
              <psxctl:DefaultValue>900</psxctl:DefaultValue>
           </psxctl:Param>
           <psxctl:Param name="dlg_height" datatype="Number" paramtype="generic">
              <psxctl:Description>This parameter specifies the height of the dialog box that is opened during field editing in Active Assembly.</psxctl:Description>
              <psxctl:DefaultValue>350</psxctl:DefaultValue>
        ...
        <psxctl:AssociatedFileList>
            ...
            <psxctl:FileDescriptor name="history.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/ps/content/History.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="selecttemplates.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/ps/content/SelectTemplates.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            ...
            <psxctl:FileDescriptor name="rx_ephox.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>/Rhythmyx/editlive/rx_ephox.jsp</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
        </psxctl:AssociatedFileList>
        <psxctl:Dependencies>
    ...
    <xsl:template match="Control[@name='sys_EditLive']" priority="10" mode="psxcontrol">
      ...
      <xsl:variable name="config_app_url">../sys_Ephox_support/config.xml&#063;config=<xsl:value-of select="$encodedConfigUrl"/>&amp;field=<xsl:value-of select="$uniqueName"/>&amp;sys_lang=<xsl:value-of select="$validItemLocale"/>&amp;isReadOnly=<xsl:value-of select="@isReadOnly"/>&amp;pssessionid=<xsl:value-of select="$pssessionid"/>
      </xsl:variable>
      ...
      <xsl:variable name="divname" select="concat('ce_',$uniqueName)"/>          
      <input type="hidden" name="{$uniqueName}" id="{$uniqueName}" value="{Value}"/>
      <textarea style="display:none;" id="{$divname}_contentArea"><xsl:value-of select="Value"/></textarea>
      <div id="{$divname}" class="editableEditLiveSection">
      <xsl:attribute name="style">height: <xsl:value-of select="$height"/><xsl:if test="not(contains($height,'%') or contains($height,'px'))">px</xsl:if>; width: <xsl:value-of select="$width"/><xsl:if test="not(contains($width,'%') or contains($width,'px'))">px</xsl:if>;</xsl:attribute>
      </div>
    ...
    <script language="JavaScript">
     ...
     $(document).ready(function(){
            var tempContent = ($("#]]><xsl:value-of select='$divname'/><![CDATA[_contentArea").val());
        $("#" + "]]><xsl:value-of select='$divname'/><![CDATA[").html(tempContent);
     
       });     

        function CustomControlIsDirty_]]><xsl:value-of select="$uniqueName"/><![CDATA[()
        {          
           return isEphoxControlDirty();
        }       
        ...
        /* End of custom javascript functions */
         ...
         ed.config = "]]><xsl:value-of select="$config_app_url"/><![CDATA["; 
         ...
         ed.lang = "]]><xsl:value-of select="$lang"/><![CDATA["; 
         ed.readOnly = "]]><xsl:value-of select="$readOnlyBoolean"/><![CDATA["; 

		function RxEphoxVariantLink_]]><xsl:value-of select="$uniqueName"/><![CDATA[()
		{
		   _activeEditLiveSectionName = "]]><xsl:value-of select="$uniqueName"/><![CDATA[";
		   RxEphoxVariantLink();
		}
       ...
       <style>
        #_editLiveInstance_appletWrapper {
            height:100%;
        }
       </style>
    </xsl:template> 
                            ...
                            <table width="100%" cellpadding="0" cellspacing="1" border="0" class="backgroundcolor">
                                <tr class="headercell">
                                    <td class="headercell2errorfont">
                                        <div id="globalErrorMessage" tabindex="0" class="headercell2errorfont">
                                    ...
                                    <tr>
                                        <td>
                                            <xsl:if test="@submitName!=''">
                                                ...
                                                <table width="100%" cellpadding="0" cellspacing="0" border="0" class="backgroundcolor"  summary="content error(s)" >
                                                    <tr>
                                                        <th width="20%" class="headererrorcell" id="error_ce_{@displayName}" scope="row">
                                                                <xsl:variable name="keyval">
                                                                    <xsl:choose>
                                                                ...
                                                                <a href="#{$subName}_error" class="headercell2errorfont">
                                                                    <xsl:call-template name="getLocaleString">
                                                                        <xsl:with-param name="key" select="$keyval"/>
                                                                        <xsl:with-param name="lang" select="$lang"/>
                                                                    </xsl:call-template>
                                                        ...
                                                        <td class="headererrorcell" headers="error_ce_{@displayName}" tabindex="0">
                                                                <xsl:if test=".!=''">
                                                                    <xsl:call-template name="getLocaleString">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <table border="0">
                                <tr>
                                    <td class="datadisplay" valign="top" align="left">
                                        <input name="{$Control/@paramName}" id="ce_{$Control/@paramName}" type="checkbox" value="{Value}">
...
<!-- END sys_CheckBoxTree -->
    ...
    <psxctl:ControlMeta name="sys_CheckBoxTreeJS" dimension="array" choiceset="required">
        ...
        <psxctl:AssociatedFileList>
            ...
            <psxctl:FileDescriptor name="ui.dynatree.css" type="css" mimetype="text/css">
                <psxctl:FileLocation>../sys_resources/js/jquery/dynatree/src/skin/ui.dynatree.css</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>                
            <psxctl:FileDescriptor name="ui.core.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/jquery/dynatree/src/jquery.dynatree.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
    ...
    <xsl:template match="Control[@name='sys_CheckBoxTreeJS']" mode="psxcontrol">
        ...
        // <![CDATA[
            ...
            var opts = {url : treeSrcUrl, selected : selectedValues, paramName : paramName, readonly : readonly};
        ...
        <div id="{@paramName}-tree">
            <xsl:attribute name="style">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                    <xsl:value-of select="concat('width:', $width, 'px; height:', $height, 'px; overflow:auto;') " />
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
   ...
   <!-- 
     sys_EditLiveDynamic
     -->
    <psxctl:ControlMeta name="sys_EditLiveDynamic" dimension="single" choiceset="none" deprecate="yes" replacewith="sys_EditLive">
        <psxctl:Description>Ephox EditLive HTML Editor</psxctl:Description>
        <psxctl:ParamList>
            <psxctl:Param name="width" datatype="String" paramtype="generic">
            ...
            <psxctl:Param name="config_src_url" datatype="String" paramtype="generic">
                <psxctl:Description>This parameter specifies the location of the config.xml that will the control will use for configuration. This file must be in the /rx_resources/ephox folder.  The default value is "config.xml".</psxctl:Description>
                <psxctl:DefaultValue>../rx_resources/ephox/elj_config_dynamic.xml</psxctl:DefaultValue>
            </psxctl:Param>
            <psxctl:Param name="config_download" datatype="String" paramtype="generic">
                <psxctl:Description>This parameter specifies the location of the download directory The default value is "../rx_resources/ephox/editlivejava".</psxctl:Description>
                <psxctl:DefaultValue>../rx_resources/ephox/editlivejava</psxctl:DefaultValue>
        ...
        <psxctl:AssociatedFileList>
            ...
            <psxctl:FileDescriptor name="inlineEditing.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../rx_resources/ephox/editlivejava/inlineEditing.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="rx_ephox.js" type="script" mimetype="text/javascript">
    ...
    <xsl:template match="Control[@name='sys_EditLiveDynamic']" mode="psxcontrol-customcontrol-isdirty">
           <xsl:variable name="parentEl" select="name(parent::node())"/>
           <xsl:if test="$parentEl != 'Column'">
             <xsl:value-of select="concat(' || CustomControlIsDirty_',@paramName,'()')"/>
          </xsl:if>
    </xsl:template>
    <xsl:template match="Control[@name='sys_EditLiveDynamic']" priority="10" mode="psxcontrol">
        <xsl:variable name="name">
            <xsl:value-of select="@paramName"/>
        </xsl:variable>
        <xsl:variable name="width">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='width']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="height">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='height']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="config_src_url">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='config_src_url']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
                <xsl:variable name="config_download">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='config_download']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="InlineLinkSlot">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='InlineLinkSlot']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="InlineImageSlot">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='InlineImageSlot']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="InlineVariantSlot">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='InlineVariantSlot']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="DebugLevel">
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:value-of select="document('')/*/psxctl:ControlMeta[@name='sys_EditLiveDynamic']/psxctl:ParamList/psxctl:Param[@name='DebugLevel']/psxctl:DefaultValue"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="encodedConfigUrl" xmlns:URLEncoder="java:java.net.URLEncoder"
      ...
      <textarea style="display:none;" id="{$divname}_contentArea"><xsl:value-of select="Value"/></textarea>
      <div id="{$divname}" class="editableEditLiveSection">
      <xsl:attribute name="style">height: <xsl:value-of select="$height"/><xsl:if test="not(contains($height,'%') or contains($height,'px'))">px</xsl:if>; width: <xsl:value-of select="$height"/><xsl:if test="not(contains($width,'%') or contains($width,'px'))">px</xsl:if>;</xsl:attribute>
      </div>
     <!-- now comes the actual script -->   
    <script language="JavaScript">
     <![CDATA[
...
>
  -->
   ...
   <xsl:template match="Control[@name='sys_DropDownMultiple']" mode="psxcontrol">
      ...
      <xsl:if test="@isReadOnly !='yes'">
         <!-- syntax checkAll(document.EditForm.$fieldnanme) -->
         <a href="#">
            <xsl:attribute name="href"><xsl:text disable-output-escaping="yes"><![CDATA[javascript:PSOselectAll(document.EditForm.]]></xsl:text><xsl:value-of select="@paramName"/><![CDATA[)]]><xsl:text/></xsl:attribute>Select All</a>
         <xsl:text>&#160; &#160;</xsl:text>
         <a href="#">
            <xsl:attribute name="href"><xsl:text disable-output-escaping="yes"><![CDATA[javascript:PSOunselectAll(document.EditForm.]]></xsl:text><xsl:value-of select="@paramName"/><![CDATA[)]]><xsl:text/></xsl:attribute>Unselect All</a>
      </xsl:if>
   </xsl:template>
 ...
 -->
    ...
    <xsl:template name="sys_hashedfilereadonly" match="Control[@name='sys_HashedFile' and @isReadOnly='yes']" priority="10" mode="psxcontrol">
        ...
        <a target="_blank" href="/Rhythmyx/assembly/aa?widget=hf&amp;hash={Value}">
            <xsl:call-template name="getLocaleString">
                <xsl:with-param name="key" select="'psx.contenteditor.sys_templates@Preview File'"/>
                <xsl:with-param name="lang" select="$lang"/>
            </xsl:call-template>
    ...
    <!--
       sys_TaxonomyAccordion
    -->
    <psxctl:ControlMeta name="sys_TaxonomyAccordion" dimension="single" choiceset="none">
        <psxctl:Description>a taxonomy tagging/browseing interface</psxctl:Description>
        <psxctl:ParamList>
              <psxctl:Param name="id" datatype="String" paramtype="generic">
                <psxctl:Description>This parameter assigns a name to an element. This name must be unique in a document.</psxctl:Description>
            </psxctl:Param>
           ...
           <psxctl:Param name="taxonomy_name" datatype="Number" paramtype="generic">
              <psxctl:Description>The name of the taxonomy this control should be related to.</psxctl:Description>
           </psxctl:Param>
        </psxctl:ParamList>
        <psxctl:AssociatedFileList>
            <psxctl:FileDescriptor type="css" name="default" mimetype="text/css">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/themes/default/style.css</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor type="css" name="default" mimetype="text/css">
                <psxctl:FileLocation>../sys_resources/css/taxonomy/cupertino/jquery-ui-1.8.12.custom-taxonomy.css</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor type="css" name="default" mimetype="text/css">
                <psxctl:FileLocation>../sys_resources/css/taxonomy/taxonomy.css</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="jquery-1.5.1.min.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/jquery-1.5.1.min.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>            
            <psxctl:FileDescriptor name="jquery-1.5.1-no-conflict.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/jquery-1.5.1-no-conflict.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="jquery-ui-1.8.12.custom-taxonomy.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/jquery-ui-1.8.12.custom-taxonomy.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="jquery.jstree.rev236-taxonomy.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/jquery.jstree.rev236-taxonomy.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
            <psxctl:FileDescriptor name="taxonomy.js" type="script" mimetype="text/javascript">
                <psxctl:FileLocation>../sys_resources/js/taxonomy/taxonomy.js</psxctl:FileLocation>
                <psxctl:Timestamp/>
            </psxctl:FileDescriptor>
        </psxctl:AssociatedFileList>
    </psxctl:ControlMeta>
    <xsl:template match="Control[@name='sys_TaxonomyAccordion']" mode="psxcontrol">
        <xsl:variable name="taxonomyId">
            <xsl:value-of select="ParamList/Param[@name='taxonomy_name']"/>
        </xsl:variable>
        <!-- ****************** Start TAXONOMY ACCORDION ****************** -->
        <!-- NAMING SHOULD BE AS FOLLOWS: -->
        <!-- tax_${name}_accordion -->
        <!-- tax_${name}_suggest -->
        <!-- tax_${name}_tree -->
        <!-- tax_${name}_tree_dropdown -->
        <!-- tax_${name}_tree_tags -->
        <!-- ${name} (starting/saved values) -->
        <div id="tax_{@paramName}_tree_accordion">
            <!-- START QUICK TAG -->
            <div>
                <h3>
                    <a href="#">
                        <xsl:value-of select="concat('Taxonomy ', @paramName,' Quick Tag')"/>
                    </a>
                </h3>
                <!-- suggest stuff -->
                <div class="tax_{@paramName}_suggest">
                    <div class="ui-widget">
                        <table border="0" width="100%">
                            <tr>
                                <td>
                                    <div id="tax_{@paramName}_tree_spinner" style="display: visible;"><img src="../sys_resources/images/taxonomy/spinner3-black.gif" alt="spinner"/> Loading...</div>
                                    <div id="tax_{@paramName}_tree_tags"/>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <label for="tax_{@paramName}_tree_dropdown">Quick Tag:</label>
                                    <input size="50" id="tax_{@paramName}_tree_dropdown"/>
                                </td>
                            </tr>
                        </table>
                    </div>
            ...
            <div>
                <h3>
                    <a href="#">
                        <xsl:value-of select="concat('Taxonomy ', @paramName,' by Browsing')"/>
                    </a>
                </h3>
                <div>
                    <input id="{@paramName}" name="{@paramName}" type="hidden" value="{Value}"><xsl:if test="@accessKey!=''">
                                <xsl:attribute name="accesskey"><xsl:call-template name="getaccesskey"><xsl:with-param name="label" select="preceding-sibling::DisplayLabel"/><xsl:with-param name="sourceType" select="preceding-sibling::DisplayLabel/@sourceType"/><xsl:with-param name="paramName" select="@paramName"/><xsl:with-param name="accessKey" select="@accessKey"/></xsl:call-template></xsl:attribute>
                            </xsl:if>
                            <xsl:call-template name="parametersToAttributes">
                                <xsl:with-param name="controlClassName" select="'sys_TaxonomyAccordion'"/>
                                <xsl:with-param name="controlNode" select="."/>
                            </xsl:call-template>
                        </input>
                    <input type="hidden" id="tax_{@paramName}_tree_tax_id" name="tax_{@paramName}_tree_tax_id" value="{$taxonomyId}"/>
                    <label for="tax_{@paramName}_tree_search">Search</label><input type="text" value="" name="tax_{@paramName}_tree_search" id="tax_{@paramName}_tree_search"/><input type="button" value="Search" onclick="$j151().taxonomy('tax_tree_search','tax_{@paramName}_tree'); return false;"/>
                    <div id="tax_{@paramName}_tree"/>
                </div>
            </div>
            <!-- END BROWSE TAG  -->
        </div>
        <script type="text/javascript">
            <xsl:value-of select="concat('$j151().taxonomy(&quot;tax_create_accordion&quot;,&quot;tax_',@paramName,'_tree&quot;,&quot;', $taxonomyId, '&quot;);','$j151().taxonomy(&quot;tax_create_tree&quot;,&quot;tax_',@paramName,'_tree&quot;,&quot;', $taxonomyId, '&quot;);')" disable-output-escaping="yes"/>           
        </script>
        <!-- debugging code
        <script type="text/javascript">
            /*
    ...
    <!-- read only template for dropdown single -->
    <xsl:template match="Control[@name='sys_TaxonomyAccordion' and @isReadOnly='yes']" priority="11" mode="psxcontrol">
        <div class="datadisplay">
            <xsl:variable name="Val" select="Value"/>
            <xsl:variable name="paramName" select="@paramName"/>
            <span id="tax_ids_to_string_{@paramName}">
                <xsl:value-of select="Value"/>
            </span>
            <input type="hidden" name="{@paramName}">
                <xsl:attribute name="value">
            ...
            <script type="text/javascript">
                <xsl:value-of select="concat('$j151.get(&quot;/Rhythmyx/user/apps/xmlprovider.htm?action=getIdsToString&amp;ids=',Value,'&quot;, function(data) {$(&quot;#tax_ids_to_string_',@paramName,'&quot;).html(data);});')" disable-output-escaping="yes"/>
            </script>
        </div>
    </xsl:template>
